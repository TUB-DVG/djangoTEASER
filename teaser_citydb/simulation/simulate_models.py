...
class WorkerSimulation(multiprocessing.Process):
    ...
    compiler : list
        1st entry: str with compiler (e.g.: "vs")
        2nd entry: list with ccompiler and path to it
            (e.g. ['CCompiler=MSVC', 'Path/to/compiler'])
    """
...
def simulate(
    ...
    for count, bldg in enumerate(sim_part):
        ...
        if count % SIMULATIONS_BEFORE_RESTART == 0:
            ...
            except DymolaConnectionException:
                print("Dymola connection error---trying to reconnect")
                time.sleep(10)
                dymola = DymolaInterface()
            if compiler is None:
                pass
            else:
                dymola.SetDymolaCompiler(compiler[0], compiler[1])
            try:
                dymola.openModel(
                ...
                dymola.openModel(
                    os.path.join(model_path, city_model.name, "package.mo")
                )
            except DymolaConnectionException:
                print("open Model failed ---trying again")
                time.sleep(10)
                try:
                    dymola.close()
                except NameError:
